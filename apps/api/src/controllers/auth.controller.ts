...
export const loginController = async (
  ...
  try {
    ...
    const { token, expiresAt } = await createSession(
      result.user.id,
      is_remember
        ? DAY_DEFAULT_SESSION_TTL * 30 * 1000
        : DAY_DEFAULT_SESSION_TTL * 1000,
    );
    res.cookie("session", token, {
      httpOnly: true,
      secure: process.env.NODE_ENV === "production",
    ...
    await SET_CasheSession(
      token,
      result.user,
      is_remember ? WEEK_DEFAULT_SESSION_TTL : DAY_DEFAULT_SESSION_TTL,
    );

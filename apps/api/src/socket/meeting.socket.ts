...
const meetingMediaHealthStates: Map<string, Map<number, MediaHealthState>> = new Map();
...
export function initializeMeetingSocket(io: Server) {
  ...
  io.on("connection", (socket: AuthenticatedSocket) => {
    ...
    socket.on("join-meeting", async ({ meetingId }) => {
      ...
      if (!meetingMediaHealthStates.has(room)) {
        meetingMediaHealthStates.set(room, new Map());
      }
      ...
      const mediaHealthStates = meetingMediaHealthStates.get(room);
      ...
      socket.emit("meeting-state", {
        ...
        mediaHealthStates: Object.fromEntries(mediaHealthStates?.entries() || []),
      });
    ...
    socket.on("leave-meeting", ({ meetingId }) => {
      ...
      if (roomMap) {
        ...
        if (roomMap.size === 0) {
          ...
          meetingMediaHealthStates.get(room)?.delete(socket.user.id);
        }
      }
    ...
    socket.on("participant-media-health", ({ meetingId, health }) => {
      const room = `meeting:${meetingId}`;
      ...
      meetingMediaHealthStates.get(room)!.set(socket.user!.id, {
        micEnabled: !!health.micEnabled,
        micReady: !!health.micReady,
        micPublished: !!health.micPublished,
        cameraEnabled: !!health.cameraEnabled,
        cameraReady: !!health.cameraReady,
        cameraPublished: !!health.cameraPublished,
        lastCheckedAt: typeof health.lastCheckedAt === "string" ? health.lastCheckedAt : new Date().toISOString(),
      });

      io.to(room).emit("participant-media-health-changed", {
        userId: socket.user!.id,
        health: meetingMediaHealthStates.get(room)!.get(socket.user!.id),
      });
    });

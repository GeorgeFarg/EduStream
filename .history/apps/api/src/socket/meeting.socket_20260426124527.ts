...
export function initializeMeetingSocket(io: Server) {
  ...
  io.on("connection", (socket: AuthenticatedSocket) => {
    ...
    socket.on("join-meeting", async ({ meetingId }: { meetingId: number }) => {
      ...
      // Track socket for this user in this meeting
      ...
      // Tell the joining user their own userId for WebRTC init
    ...
    socket.on(
      "webrtc-offer",
      ({ meetingId, targetUserId, offer }: { meetingId: number; targetUserId: number; offer: RTCSessionDescriptionInit }) => {
        if (!socket.user) return;
        const room = `meeting:${meetingId}`;
        const roomMap = meetingUserSockets.get(room);
    ...
    socket.on(
      "webrtc-answer",
      ({ meetingId, targetUserId, answer }: { meetingId: number; targetUserId: number; answer: RTCSessionDescriptionInit }) => {
        if (!socket.user) return;
        const room = `meeting:${meetingId}`;
        const roomMap = meetingUserSockets.get(room);
        const targetSocketId = roomMap?.get(targetUserId);
    ...
    socket.on(
      ...
      ({ meetingId, targetUserId, candidate }: { meetingId: number; targetUserId: number; candidate: RTCIceCandidateInit }) => {
        if (!socket.user) return;
        const room = `meeting:${meetingId}`;
        const roomMap = meetingUserSockets.get(room);
        const targetSocketId = roomMap?.get(targetUserId);
    ...
    socket.on("disconnect", () => {
      ...
      // Clean up user from all meeting rooms

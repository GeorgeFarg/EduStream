  ...
  setRemoteStreams((prev) => ({
    ...
    [targetUserId]: remoteStream,
      ...
      if (selfId !== null && selfId < targetUserId) {
        // ✅ لا تستخدم onnegotiationneeded — ابدأ الـ offer يدوياً بعد إضافة الـ tracks
        ...
        // ✅ ابدأ الـ offer بس لو في tracks، غير كده استنى negotiationneeded
        if (localStreamRef.current && localStreamRef.current.getTracks().length > 0) {
          startOffer();
        } else {
          pc.onnegotiationneeded = startOffer;
        }
      }
...
const initLocalStream = useCallback(
    (stream: MediaStream) => {
      ...
      Object.entries(peerConnectionsRef.current).forEach(([userIdStr, pc]) => {
        ...
        // ✅ بعد إضافة الـ tracks، ابدأ offer لو الـ self ID أصغر
        const selfId = currentUserIdRef.current;
        if (selfId !== null && selfId < userId) {
          pc.createOffer()
            ...
            .catch((err) => console.error("Error renegotiating after stream init:", err));
        }
      });
    ...
    [participants, initPeerConnection, meetingId]
  );

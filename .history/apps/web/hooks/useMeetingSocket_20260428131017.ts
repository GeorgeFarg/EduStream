...
export function useMeetingSocket(meetingId: number | null) {
  ...
  }, [cleanupPeerConnections, isConnected, meetingId]);
  ...
  const initLocalStream = useCallback(
    ...
    [initPeerConnection, renegotiatePeer]
  );

  const replaceVideoTrack = useCallback(
    (newTrack: MediaStreamTrack | null, options?: { forceNewSender?: boolean }) => {
      Object.entries(peerConnectionsRef.current).forEach(([userIdStr, pc]) => {
        const targetUserId = Number(userIdStr);
        const sender = pc.getSenders().find((s) => s.track?.kind === "video");
        const forceNewSender = options?.forceNewSender ?? false;

        if (sender && !forceNewSender) {
          sender

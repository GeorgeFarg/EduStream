...
// Avatar color palette for visual variety
const AVATAR_COLORS = [
...
export default function ClassMeeting({ classId, className }: { classId: string; className?: string }) {
  ...
  // Socket hook for real-time meeting state + WebRTC
  const {
    currentUserId,
  ...
  // Handle meeting ended from socket
  useEffect(() => {
  ...
  const joinMeetingRoom = async (meetingId: number) => {
    try {
      ...
      if (res.ok) {
        ...
        // Initialize WebRTC after a short delay to ensure socket is connected
        setTimeout(() => {
          if (stream) {
  ...
  // Build participant list for grid display
    ...
    return (
      <div className="h-full flex flex-col bg-[#0a0a0a]">
        ...
        <div className="flex-1 flex overflow-hidden">
          {/* Video Grid */}
          <div className="flex-1 bg-black p-2 md:p-4 overflow-y-auto">
                      ...
                      <video
                        ref={(el) => {
                          if (participant.isLocal) {
                            // local video handled separately
                          } else {
                        ...
                        className={`w-full h-full object-cover ${isScreenShare ? "object-contain" : ""}`}
                      />
                    ) : (
            ...
            {/* Hidden local video for self preview */}
            <video
              ref={localVideoRef}
              autoPlay
              playsInline
